...
const DarkModeToggle = ({ className = "" }) => {
  ...
  if (isLoading) {
    return (
      <div className={`relative p-2 w-9 h-9 ${className}`}>
  ...
  return (
    <button
      ...
      className={`relative group p-2 rounded-xl bg-gray-100/80 dark:bg-gray-800/80 hover:bg-gray-200/80 dark:hover:bg-gray-700/80 transition-all duration-300 ease-in-out border border-gray-200/50 dark:border-gray-700/50 shadow-sm hover:shadow-lg backdrop-blur-sm ${className}`}
    ...
    >
      <div className="relative w-5 h-5 overflow-hidden">
        {/* Sun Icon */}
        <Sun 
          className={`absolute inset-0 w-5 h-5 text-amber-500 transition-all duration-500 ease-in-out transform ${
            isDarkMode 
        ...
        {/* Moon Icon */}
        <Moon 
          className={`absolute inset-0 w-5 h-5 text-indigo-400 transition-all duration-500 ease-in-out transform ${
            isDarkMode 
              ? 'rotate-0 scale-100 opacity-100 translate-y-0' 
              : '-rotate-180 scale-0 opacity-0 -translate-y-2'
          }`}
